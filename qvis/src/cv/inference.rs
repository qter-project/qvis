...
impl Inference {
    pub(crate) fn from_assignment(
        ...
        let colors: Box<[_]> = assignment.stickers_by_face.keys().cloned().collect();
        ...
        for (idx, pixel) in assignment.pixels.into_iter().enumerate() {
            ...
            pixels_by_sticker[sticker].push(Pixel { idx, kdtrees });
        }
            ...
            .map(|v| {
                ...
                for pixel in v {
                    for (color, kdtree) in &pixel.kdtrees {
                        ...
                        let n = MAX_NEAREST_N.min(kdtree.size() as usize / MAX_FRACTION).max(1);
                        ...
                        let density = n as f64 / kdtree.size() as f64 * (nn.last().unwrap().distance.powi(3) * UNIT_SPHERE).recip();
...
mod tests {
    ...
    use crate::cv::inference::quickselect;
